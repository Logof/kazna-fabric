...
module.exports = {
  getAll: async (req, res, next) => {
    try {
      ...
      const result = await contract.evaluateTransaction('ListZKR');
      res.json({status: true, ZKR: JSON.parse(result.toString())});
    } catch (err) {
  ...
  putZKR: async (req, res, next) => {
    ...
    try {
      ...
      console.log("------------------------------------ Начало ------------------------------------")
      let resp = await contract.submitTransaction('PutZKR', jsonString);
      console.log("Ответ:" + resp);
      res.json({status: true, message: 'Transaction (create ZKR) has been submitted.'})
      console.log("------------------------------------ Конец ------------------------------------")
    } catch (err) {
      console.log("Catch Exception: " + err)
      res.json({status: false, error: err});
