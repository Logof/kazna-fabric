...
module.exports = {
  getAll: async (req, res, next) => {
    try {
      ...
      const result = await contract.evaluateTransaction('ListDO');
      res.json({status: true, DO: JSON.parse(result.toString())});
    } catch (err) {
  ...
  putDO: async (req, res, next) => {
    ...
    try {
      ...
      console.log("------------------------------------ Начало ------------------------------------")
      let resp = await contract.submitTransaction('PutDO', jsonString);
      console.log("Ответ:" + resp);
      res.json({status: true, message: 'Transaction (create DO) has been submitted.'})
      console.log("------------------------------------ Конец ------------------------------------")
    } catch (err) {
      console.log("Catch Exception: " + err)
      res.json({status: false, error: err});
