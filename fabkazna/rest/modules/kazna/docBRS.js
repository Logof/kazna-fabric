...
module.exports = {
  getAll: async (req, res, next) => {
    try {
      ...
      const result = await contract.evaluateTransaction('ListRR');
      res.json({status: true, BRS: JSON.parse(result.toString())});
    } catch (err) {
  ...
  putBRS: async (req, res, next) => {
    ...
    try {
      ...
      console.log("------------------------------------ Начало ------------------------------------")
      let resp = await contract.submitTransaction('PutRR', jsonString);
      console.log("Ответ:" + resp);
      res.json({status: true, message: 'Transaction (create BRS) has been submitted.'})
      console.log("------------------------------------ Конец ------------------------------------")
    } catch (err) {
      console.log("Catch Exception: " + err)
      res.json({status: false, error: err});
